...
class Boleto:
    ...
    def getBoletoClass(move_line):
        bank_code = move_line.src_bank_account_id.bank_id.bic
    ...
    def _move_line(self, move_line):
        ...
        self.boleto.data_documento = datetime.date(datetime.strptime(
            move_line.emission_date, '%Y-%m-%d'))
        self.boleto.data_processamento = date.today()
        self.boleto.valor = "%.2f" % move_line.amount_total
        self.boleto.valor_documento = "%.2f" % move_line.amount_total
        self.boleto.especie = \
            move_line.currency_id and move_line.currency_id.symbol or 'R$'
        self.boleto.quantidade = '1'
        self.boleto.numero_documento = move_line.name
...
class BoletoBB(Boleto):
    def __init__(self, move_line, nosso_numero):
        ...
        self.account_number = move_line.src_bank_account_id.acc_number
        self.branch_number = move_line.src_bank_account_id.bra_number
...
class BoletoBradesco(Boleto):
    def __init__(self, move_line, nosso_numero):
        ...
        self.branch_number = move_line.src_bank_account_id.bra_number
        # bank specific
        self.account_digit = move_line.src_bank_account_id.acc_number_dig
        self.branch_digit = move_line.src_bank_account_id.bra_number_dig
        # end bank specific
        Boleto.__init__(self, move_line, nosso_numero)
        self.boleto.nosso_numero = self.nosso_numero
...
class BoletoCaixa(Boleto):
    def __init__(self, move_line, nosso_numero):
        self.boleto = Boleto.getBoletoClass(move_line)()
        conta = move_line.src_bank_account_id
...
class BoletoSantander(Boleto):
    def __init__(self, move_line, nosso_numero):
        self.boleto = Boleto.getBoletoClass(move_line)()
        self.account_number = move_line.src_bank_account_id.acc_number[:7]
        self.branch_number = move_line.src_bank_account_id.bra_number
        Boleto.__init__(self, move_line, nosso_numero)
        self.boleto.nosso_numero = self.nosso_numero
...
class BoletoSicoob(Boleto):
    def __init__(self, move_line, nosso_numero):
        ...
        self.boleto.codigo_beneficiario = \
            re.sub('[^0-9]', '',
                   move_line.src_bank_account_id.codigo_convenio)
        self.boleto.nosso_numero = self.nosso_numero

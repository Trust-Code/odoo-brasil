...
class SpedEfdContribuicoes(models.Model):
    ...
    def create_file(self):
        if self.date_start > self.date_end:
            raise UserError(
                'Erro, a data de início é maior que a data de encerramento!')
    ...
    def registro0000(self):
        arq = ArquivoDigital()
        cod_mun = '%s%s' % (self.company_id.state_id.ibge_code,
        ...
        arq._registro_abertura.CNPJ = self.limpa_formatacao(
        ...
        # Cód. ind. de método de apropriação de créditos comuns
        reg110.IND_APRO_CRED = self.ind_apro_cred
        # Cód. ind. do Tipo de Contribuição Apurada
        reg110.COD_TIPO_CONT = self.cod_tipo_cont
        # Cód. ind. do critério de escrituração e apuração adotado
        reg110.IND_REG_CUM = self.ind_reg_cum
        ...
        # arqxx.close()
        self.sped_file = base64.encodestring(
            bytes(arq.getstring(), 'iso-8859-1'))
    ...
    def query_registro0150(self, periodo):
        ...
        for id in query_resposta:
            ...
            cod_mun = '%s%s' % (resposta_participante.state_id.ibge_code,
                                resposta_participante.city_id.ibge_code)
            if cod_pais == '01058':
                ...
                if len(cpnj_cpf) == 11:
                    ...
                    registro_0150.IE = self.limpa_formatacao(
                        resposta_participante.inscr_est)
            else:
                registro_0150.COD_MUN = '9999999'
            registro_0150.SUFRAMA = self.limpa_formatacao(
                resposta_participante.suframa)
    ...
    def query_registro0190(self, periodo):
        ...
        for id in query_resposta:
            ...
            desc = id[1]
            if not desc:
                msg_err = 'Unidade de medida sem descricao - Un %s.' % (
                    unidade)
                raise UserError(msg_err)
    ...
    def query_registro0200(self, periodo):
        ...
        for resposta in query_resposta:
            ...
            registro_0200.COD_NCM = self.limpa_formatacao(
                resposta_produto.fiscal_classification_id.code)
    ...
    def query_registro0400(self, periodo):
        ...
        lista = []
        for resposta in query_resposta:
            resposta_nat = self.env['account.fiscal.position'].browse(
                resposta[0])
        ...
        for nf in nfe_ids:
            if (nf.state == 'done') and (nf.model == '55'):
                ...
                if not cancel:
                    ...
                    registro_c100.VL_MERC = self.transforma_valor(
                        nf.valor_bruto)
                    registro_c100.IND_FRT = str(nf.modalidade_frete)
                    registro_c100.VL_FRT = self.transforma_valor(
                        nf.valor_frete)
                    registro_c100.VL_SEG = self.transforma_valor(
                        nf.valor_seguro)
                    registro_c100.VL_OUT_DA = self.transforma_valor(
                        nf.valor_despesas)
                    registro_c100.VL_DESC = self.transforma_valor(
                        nf.valor_desconto)
                    registro_c100.VL_DOC = self.transforma_valor(
                        nf.valor_final)
                    registro_c100.VL_BC_ICMS = self.transforma_valor(
                        nf.valor_bc_icms)
                    registro_c100.VL_ICMS = self.transforma_valor(
                        nf.valor_icms)
                    registro_c100.VL_BC_ICMS_ST = self.transforma_valor(
                        nf.valor_bc_icmsst)
                    registro_c100.VL_ICMS_ST = self.transforma_valor(
                        nf.valor_icmsst)
                    registro_c100.VL_IPI = self.transforma_valor(nf.valor_ipi)
                    registro_c100.VL_PIS = self.transforma_valor(nf.valor_pis)
                    registro_c100.VL_COFINS = self.transforma_valor(
                        nf.valor_cofins)
    ...
    def query_registroC170(self, doc):
        ...
        for item in nfe_line:
            ...
            try:
                registro_c170.CST_ICMS = '%s%s' % (
                    str(item.origem), str(item.icms_cst))
            except:
            ...
            # if r_nfe.id == 407:
            #    import pudb;pu.db
            registro_c170.COD_NAT = str(
                item.invoice_eletronic_id.fiscal_position_id.id)
            registro_c170.VL_BC_ICMS = self.transforma_valor(
            ...
            registro_c170.VL_BC_ICMS_ST = self.transforma_valor(
                item.icms_st_base_calculo)
            if item.icms_st_aliquota:
                registro_c170.ALIQ_ST = self.transforma_valor(
                    item.icms_st_aliquota)
            registro_c170.VL_ICMS_ST = self.transforma_valor(
                item.icms_st_valor)
            # TODO incluir na empresa o IND_APUR
            registro_c170.IND_APUR = '0'
            registro_c170.CST_IPI = item.ipi_cst
            registro_c170.VL_BC_IPI = self.transforma_valor(
                item.ipi_base_calculo)
            if item.ipi_aliquota:
                registro_c170.ALIQ_IPI = self.transforma_valor(
                    item.ipi_aliquota)
            registro_c170.VL_IPI = self.transforma_valor(item.ipi_valor)
            registro_c170.CST_PIS = item.pis_cst
            registro_c170.VL_BC_PIS = self.transforma_valor(
                item.pis_base_calculo)
            ...
            registro_c170.VL_BC_COFINS = self.transforma_valor(
                item.cofins_base_calculo)
            registro_c170.ALIQ_COFINS = self.transforma_valor(
    ...
    def query_registroD100(self, doc):
        ...
        for resposta in resposta_cte:
            cte = self.env['invoice.eletronic'].search(
                [('invoice_id', '=', fatura)])
            ...
            # or resposta_nfe.product_document_id.code).zfill(2)
            registro_d100.COD_MOD = str(resposta.nfe_modelo)
            ...
            """
            registro_d100.SER = resposta.nfe_serie[:
                                                   3]  # resposta.product_serie_id.code
            if resposta.nfe_chave:
                if len(resposta.nfe_chave) != 44:
                    msg_err = 'Tamanho da Chave NFe invalida - Fatura %s.' % (
                        str(resposta.number or resposta.id))
                    #raise UserError(msg_err)
                    self.log_faturamento += msg_err
            # or resposta_nfe.chave_nfe
            registro_d100.CHV_CTE = str(resposta.nfe_chave)
            registro_d100.NUM_DOC = self.limpa_formatacao(
                str(cte.numero))  # or resposta_nfe.numero))
            ...
            registro_d100.VL_DESC = self.transforma_valor(
                resposta.total_desconto)
            registro_d100.IND_FRT = '1'  # Destinatario
            registro_d100.VL_SERV = self.transforma_valor(
                resposta.amount_total)
            registro_d100.VL_BC_ICMS = self.transforma_valor(
                resposta.icms_base)

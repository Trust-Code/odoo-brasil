...
class AccountMove(models.Model):
    ...
    def action_post(self):
        moves = self.filtered(
          lambda x: x.l10n_br_edoc_policy in ('directly','after_payment') and x.move_type != 'entry'
        )
    ...
    def get_peso_bruto(self):
        if self.company_id.l10n_br_automated_weight:
            ...
            package_weight = sum(
                picking_ids.mapped(
                    "move_ids_without_package.move_line_ids.result_package_id.packaging_id.package_weight"
                )
            )
